...
class PresensiController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        // Cek Konfigurasi Absensi Bebas
        $konfigurasi_lokasi = DB::table('konfigurasi_lokasi')->where('kode_cabang', $kode_cabang)->first();
        $bebas_absen = $konfigurasi_lokasi ? $konfigurasi_lokasi->bebas_absen : false;
        ...
        if ($status_location == 1 && $radius > $lok_kantor->radius_cabang) {
            ...
            if ($cek > 0) {
                if ($bebas_absen == false && $jam_pulang < $jamkerja_pulang) {
                ...
                $bisa_absen_masuk = true;
                if($bebas_absen == false){
    ...
    public function updateprofile(Request $request)
    {
        ...
        if ($update) {
            ...
            return Redirect::back()->with(['error' => 'data gagal di update silakan masukan data secara lengkap']);
        }
    }
    ...
    public function batalkanizinsakit($kode_izin)
    {
        ...
        if ($update) {
            ...
            return Redirect::back()->with(['warning' => 'data gagal di update silakan masukan data secara lengkap']);
        }
    }
